...
export class NodeService {
  ...
  async update(nodes: Node[]) {
    ...
    const valueStrings = nodes.map((node: Node, indexNode: number) => {
    ...
    SET
      ...
      FROM (VALUES ${Prisma.join(valueStrings.map((row) => Prisma.sql`(${Prisma.join(row)})`))}
    ...
    `
    await prisma.$executeRaw(excute)
